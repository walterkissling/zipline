...
class OpenInterestRollFinder(RollFinder):
    """
    The CalendarRollFinder calculates contract rolls based on when
    volume activity transfers from one contract to another.
    """
    ...
    def _active_contract(self, oc, front, back, dt):
        ...
        front_vol = get_value(front, prev, 'open_interest')
        back_vol = get_value(back, prev, 'open_interest')
        if back_vol > front_vol:
            return back
        ...
        for session in sessions:
            front_vol = get_value(front, session, 'open_interest')
            back_vol = get_value(back, session, 'open_interest')
            if back_vol > front_vol:
                return back
        return front
